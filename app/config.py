...
class Settings(BaseSettings):
    ...
    # Бюджет ожидания ответа агента (read-таймаут httpx в фоновом прогоне,
    # hermes_service.py). Чат асинхронный (фаза 27): /agent/send отвечает сразу,
    # а прогон ждёт фоновая задача, поэтому агентные запросы с веб-поиском
    # легитимно идут десятки минут. Соединение/запись обрываются быстрее (см.
    # hermes_service.py — раздельный httpx.Timeout).
    HERMES_TIMEOUT: int = 2400

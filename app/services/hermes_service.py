...
async def send_to_hermes(
    ...
    if search_mode == "internet":
        ...
        system_content = base + (
            f"РЕЖИМ ПОИСКА: INTERNET.\n"
            f"ПРИОРИТЕТ 1 (самый высокий) — ЗАПРОСЫ ПО ИНН/ОГРН контрагента "
            f"(реквизиты, проверить компанию, «это холдинг?», учредители, статус, "
            f"КПП/ОГРН): используй ТОЛЬКО инструменты lookup_company_by_inn и "
            f"find_affiliated_companies (DaData). Они отвечают за ~1с с полными "
            f"данными. После них НЕ вызывай web_search/web_extract/browser — ответ "
            f"уже готов, сразу формулируй ответ пользователю. Сайты-реестры НЕ открывай "
            f"(виснут по 60с из-за антиспама).\n"
            f"ПРИОРИТЕТ 2 — прочие запросы: ОТВЕЧАЙ на основе веб-поиска через "
            f"web_search, web_extract, browser. Даже если запрос звучит как поиск "
            f"компании/контакта/телефона (но БЕЗ ИНН) — ищи в интернете. Источники "
            f"цитируй (сайт, ссылка). Если web_search вернул ошибку/пусто — попробуй "
            f"browser или переформулируй запрос.\n"
            f"Инструменты CRM (search_leads и др.) НЕ используй, кроме случая, когда "
            f"пользователь ЯВНО написал «в нашей базе», «в CRM», «у нас»."
        )
    else:
